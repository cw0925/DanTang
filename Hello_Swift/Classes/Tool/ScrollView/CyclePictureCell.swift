    ...
    var imageSource: ImageSource = ImageSource.local(name: ""){
        didSet {
            ...
            case let .network(urlStr):
                ...
                self.imageView.kf.setImage(with: URL(string: encodeString)!, placeholder: placeholderImage, options: [.transition(.fade(1))], progressBlock: { (receivedSize, totalSize) in
